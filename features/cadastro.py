...
def _load_oms_df():
    """Carrega da planilha as OMs e Diretorias já cadastradas."""
    try:
        df = read_df("ACOMPANHAMENTO VISTORIAS")
        if "OM" in df.columns and "diretoria" in df.columns:
            return df[["OM", "diretoria"]].dropna().drop_duplicates()
    except Exception:
        pass
...
def _input_row(oms_df: pd.DataFrame):
    ...
    with col1:
        om_solicitante = st.selectbox(
            "OM solicitante (sigla)",
            options=oms_df["OM"].unique().tolist() if not oms_df.empty else [],
            index=None,
            placeholder="Selecione ou digite..."
        )
        # Diretoria vinculada (auto)
        diretoria = ""
        if om_solicitante and not oms_df.empty:
            match = oms_df.loc[oms_df["OM"] == om_solicitante, "diretoria"]
            if not match.empty:
                diretoria = str(match.iloc[0])
    ...
    row = {
        ...
        "om_solicitante": om_solicitante or "",
